...
architecture Behavioral of dilithium_top is
  ...
  signal shake_squeeze_start_r : std_logic := '0'; -- internal 1-cycle pulse reg
  ...
  signal prev_top_state : top_state_type := WAIT_CMD;
...
begin
  ...
  shake_inst : shake256_core
    port map (
      ...
      squeeze_start => shake_squeeze_start_r,
      outlen => shake_outlen,
  ...
  -- map expand_done to done_02
  done_02 <= expand_done_sig;
  ...
  ----------------------------------------------------------------------------
  -- For any not-connected done signals (06..0B etc) tie to '0' as default
  ----------------------------------------------------------------------------
  done_06 <= '0';
  done_07 <= '0';
  done_08 <= '0';
  ...
  done_sel <= done_01 when cmd_reg = x"01" else
              done_02 when cmd_reg = x"02" else
  ...
  ----------------------------------------------------------------------------
  -- Helper: convert polyvec to 512-bit std_logic_vector
  -- NOTE: This is a placeholder returning zeros; replace with a real
  -- packing routine that maps your polyvec_t layout into 512 bits.
  ----------------------------------------------------------------------------
  function polyvec_to_sv512(v : polyvec_t(0 to L_const-1, 0 to N_const-1)) return std_logic_vector is
    variable tmp : std_logic_vector(511 downto 0) := (others => '0');
  begin
    -- TODO: implement real packing from polyvec_t -> 512-bit vector
    -- For now returns zeros so design compiles; replace with actual mapping.
    return tmp;
  end function;

  function polyvec_to_sv512_k(v : polyvec_t(0 to K_const-1, 0 to N_const-1)) return std_logic_vector is
    variable tmp : std_logic_vector(511 downto 0) := (others => '0');
  begin
    -- TODO: implement real packing from polyvec_t -> 512-bit vector
    return tmp;
...
------------------------------------------------------------------------
    ...
    begin
      ...
      case cmd_reg is
        ...
        when x"03" =>
          -- convert polyvecl NTT output into 512-bit (placeholder)
          dout_sel <= polyvec_to_sv512(pv_l_out);
        when x"04" =>
          dout_sel <= polyvec_to_sv512_k(pv_k_out);
        when x"05" =>
  ...
  ----------------------------------------------------------------------------
  -- Generate 1-cycle pulse for SHAKE squeeze_start when entering START_CORE for cmd 0x01
  ----------------------------------------------------------------------------
  shake_squeeze_pulse_proc : process(clk, reset)
  begin
    if reset = '1' then
      shake_squeeze_start_r <= '0';
      prev_top_state <= WAIT_CMD;
    elsif rising_edge(clk) then
      -- default clear
      shake_squeeze_start_r <= '0';

      -- detect rising into START_CORE
      if (top_state = START_CORE and prev_top_state /= START_CORE) then
        if cmd_reg = x"01" then
          -- issue 1-cycle squeeze_start pulse
          shake_squeeze_start_r <= '1';
        end if;
      end if;
      ...
      case top_state is
        ...
        when WAIT_DONE =>
          ...
          if done_sel = '1' then
            ...
            -- optional: simple timeout or error handling could be added here
            null;
          end if;
